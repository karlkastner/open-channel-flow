...
% J = d/dh(dh/dt)
function [J, bl] = jacobian_x(obj,t,h,p,varargin)
	if (obj.opt.analytic_jacobian)
		h = obj.input_factor*h;
		obj.interface_values_x(t,h,false,true);

		dx = obj.dx;
		z=zeros(prod(obj.nx),1);	
		J  = -[   -flat(obj.aux.dqxi_dhl(1:end-1,:))/dx(1), ... ...
                           flat(obj.aux.dqxi_dhc(2:end,:))/dx(1), ...
                           z, ... %0.*-flat(obj.aux.dqj_dhl(:,1:end-1))'/dx(2); ...
                           z, ... %0.*flat(obj.aux.dqj_dhc(:,2:end))'/dx(2)
		           flat(  ( -obj.aux.dqxi_dhc(1:end-1,:) + obj.aux.dqxi_dhl(2:end,:)) / dx(1) ...
			       ... %+ ( -obj.aux.dqj_dhc(:,1:end-1) + obj.aux.dqj_dhl(:,2:end)) / dx(2) ...
			       )
			];
		...
		h = reshape(h,obj.nx);
		J = jacobian_regular_grid_2d_i(@obj.dh_dt_i,0,h,obj.n,obj.eps,obj.twosided);
	end
end % jacobian_x
