% Sat  2 May 15:45:54 +08 2020
%
function [h,C,u] = discharge2stage(Q,width,S,d50_mm,d90_mm,T_C,profile,method)
	p       = 1;
	maxiter = 100;
	reltol  = 1e-3;
...
%	C = 5*ones(size(h));
	...
	while (true)
		...
		u    = q./h;
		rms_ = rms(h(fdx)-h_old(fdx));
		if ( rms_<h*reltol*rms(h(fdx)))
			break;
		end	
		if (~isfinite(rms_))
